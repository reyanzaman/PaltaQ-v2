...
export default function QuestionBox({
  ...
  const handleSubmit = async (e: any) => {
    ...
    const response = await fetch(
      `/api/questions?question=${encodeURIComponent(question)}&tid=${encodeURIComponent(selectedTopicId)}&tname=${encodeURIComponent(selectedTopic)}&cid=${encodeURIComponent(classId)}&cCode=${encodeURIComponent(classCode)}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      ...
      console.error("Failed to submit question (Frontend)", responseData);
  ...
  const toggleAnonymity: React.MouseEventHandler<HTMLButtonElement> = (
    event,
  ) => {
  ...
  return (
    <form className="lg:w-[90%] lg:px-3 pt-2" onSubmit={handleSubmit}>
      ...
      {/* Question Box */}
      <div className="lg:w-[100%] w-[93%] mx-auto relative lg:pl-3">
        <div className="mb-6">
          <div className="relative">
            ...
            <div className="absolute bottom-[2.5em] right-[2.5em] scale-[1.5] hidden lg:flex items-center gap-1 z-10">
              <button
                ...
                aria-label={
                  isAnonymous ? "Disable anonymity" : "Enable anonymity"
                }
      ...
      <div className="lg:pb-3 pb-1">
        {loading ? (
          ...
          <div>
            <h5 className="lg:px-3 px-2 lg:text-lg text-base">
              Progress to next ranking:{" "}
              {Math.round(progress) ? `${Math.round(progress)}%` : "Loading"} (
              {progressNum ? progressNum : ""})
            </h5>
            <div
              className="progress progress-xl lg:w-[96%] lg:mx-3 mx-2"
              style={{ height: "1.5em" }}
            >
              <div
