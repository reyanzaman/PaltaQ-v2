...
export default function QuestionBox({ onQuestionSubmitted }: { onQuestionSubmitted: any }) {
  ...
  return (
    <form className="lg:w-[100%] w-[93%] mx-auto" onSubmit={handleSubmit}>
      ...
      {visibility && (
        <div className='w-full card border-light shadow-md px-3 py-3' style={{ backgroundColor: '#0081FB' }}>
          <div>
            <div className='flex flex-row items-start justify-between'>
              <p className='text-white my-0 font-bold'>Question Improvement Suggestion</p>
              <p className='font-bold text-white cursor-pointer' onClick={() => setVisibility(false)}>Close</p>
            </div>
            
            <p className='text-white mt-3 mb-2'>{lastQuestion}</p>
            <p className={`text-white mt-2 mb-2}`}>
              {/* @ts-ignore */}
              {response ? response.improvement_suggestion : 'Waiting for response...'}
            </p>

            <span className="flex flex-row items-end justify-end">
              <h6 className="text-sm text-white mb-0 mt-4">Generated by Meta AI</h6>
            </span>
          </div>
        </div>
      )}
